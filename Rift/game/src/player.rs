...
impl Player {
    ...
    /// World-space position of the player. Reads the (root) player entity's
    /// transform; exposed so the headless demo/test harness can assert on
    /// movement without rendering.
    pub fn position(&self, transforms: &Transforms) -> Vec3f {
        transforms
            .get_absolute(self.id)
            .map_or_else(Vec3f::zero, |transform| transform.disp)
    }
...
impl<'context> InfallibleSystem<'context> for Player {
    ...
    fn update(&mut self, deps: Dependencies) {
        ...
        // Sync position with multiplayer server (no-op when offline)
        let forward = transform.rot.rotate_vector(Vec3f::new(0.0, 0.0, -1.0));
