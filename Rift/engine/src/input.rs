...
impl Input {
    ...
    /// Programmatically set a key's held state, bypassing the windowing system.
    ///
    /// Used by the headless demo/test harness to drive the game with a scripted
    /// input sequence instead of live keyboard events. A held key satisfies
    /// `Gesture::KeyHold`; pressing it on the current update also satisfies
    /// `Gesture::KeyTrigger` for that one step.
    pub fn set_key(&mut self, code: Scancode, pressed: bool) {
        self.keyboard_state[code as usize] = if pressed {
    ...
    /// Programmatically set a mouse button's held state (see [`Input::set_key`]).
    pub fn set_mouse_button(&mut self, button: MouseButton, pressed: bool) {
        let index = mouse_button_to_index(button);
        self.mouse_button_state[index] = if pressed {
            ButtonState::Down(self.current_update_index)
        } else {
            ButtonState::Up(self.current_update_index)
        };
    ...
    /// Inject relative mouse motion for the next polled step (scripted look).
    pub fn add_mouse_motion(&mut self, dx: f32, dy: f32) {
        self.mouse_rel += Vec2f::new(dx, dy);
    }
