...
impl RenderPipeline {
    ...
    /// The entity currently used as the render camera, if any.
    pub fn camera(&self) -> Option<EntityId> {
        self.camera
    }

    /// Detach the camera so the renderer draws nothing. Used by the title screen
    /// to take over the frame before gameplay begins; restore with `set_camera`.
