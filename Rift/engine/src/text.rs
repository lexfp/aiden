...
use glium::{
    implement_vertex, uniform, Blend, DrawParameters, Program, Surface, VertexBuffer,
};
...
impl TextRenderer {
    /// Insert text with its top-left corner at `pos` (normalized 0..1), at the
    /// default font size.
    pub fn insert(&mut self, win: &Window, text: &str, pos: Pnt2f, padding: u32) -> TextId {
        self.insert_sized(win, text, pos, padding, POINT_SIZE)
    }

    /// Insert text with its top-left corner at `pos`, rasterised at `point_size`
    /// (larger sizes stay crisp — used for the title screen's big type).
    pub fn insert_sized(
        &mut self,
        win: &Window,
        text: &str,
        ...
        let (width, height) = self.rasterise(text, padding, point_size);
        let h = height as f32 / win.height() as f32 * 2.0;
        let (x, y) = (pos.x * 2.0 - 1.0, 1.0 - pos.y * 2.0 - h);
        self.finish_insert(win, width, height, x, y)
    }

    /// Insert text centered on `center` (normalized 0..1), rasterised at
    /// `point_size`. Convenient for centered title/menu text.
    pub fn insert_centered(
        ...
        let (width, height) = self.rasterise(text, padding, point_size);
        let w = width as f32 / win.width() as f32 * 2.0;
        let h = height as f32 / win.height() as f32 * 2.0;
        let (x, y) = (center.x * 2.0 - 1.0 - w / 2.0, 1.0 - center.y * 2.0 - h / 2.0);
        self.finish_insert(win, width, height, x, y)
    }

    /// Build the GPU texture (from the just-rasterised `pixel_buffer`) and quad
    /// for a piece of text whose bottom-left corner is at NDC `(x, y)`.
    fn finish_insert(&mut self, win: &Window, width: u32, height: u32, x: f32, y: f32) -> TextId {
        debug!("Creating text...");
    ...
    pub fn render<S: Surface>(&self, frame: &mut S) -> Result<()> {
    ...
    fn rasterise(&mut self, text: &str, padding: u32, point_size: f32) -> (u32, u32) {
