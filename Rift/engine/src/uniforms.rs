...
use glium::texture::{
    ClientFormat, MipmapsOption, PixelValue, RawImage2d, Texture2d as GliumTexture2d,
};
...
impl Uniforms {
    ...
    pub fn add_texture_2d<'a, PixelT: PixelValue>(
        ...
        // NoMipmap is critical for the palette and the index atlases: these are
        // sampled with Nearest filtering and (for the palette) via a *dependent*
        // lookup whose coordinate jumps between neighbouring fragments. With
        // auto-generated mipmaps, the large implied level-of-detail makes some
        // drivers sample a high mip — the average of all palette colours, i.e. a
        // flat grey — turning the whole world monochrome. Without mip levels
        // there is nothing but the correct base level to sample, on any driver.
        let gl = GliumTexture2d::with_mipmaps(
            window.facade(),
            RawImage2d {
