...
export function computeSessionStartHookUpdate(settings, spec) {
    ...
    for (const group of updated.hooks.SessionStart) {
        ...
        for (const hook of group.hooks) {
            ...
            const isCorrect = hook.command === spec.command &&
                hook.type === "command" &&
...
function installOpenCodeAmbientPlugin(home, marker, command, timeoutSeconds, onError) {
    const pluginPath = join(home, ".config", "opencode", "plugins", `axi-${sanitizeOpenCodePluginFilePart(marker)}.js`);
    const managedMarker = `${OPENCODE_PLUGIN_MANAGED_PREFIX} ${marker}`;
...
export function resolvePortableHookCommand(execPath, binaryNames, marker, context) {
...
export function shouldInstallHooksForNodeAxiExecPath(execPath, policy) {
...
export function installSessionStartHooks(options = {}) {
    ...
    const shouldInstall = options.shouldInstall ??
    ...
    installOpenCodeAmbientPlugin(home, marker, command, options.timeoutSeconds ?? 10, options.onError);
