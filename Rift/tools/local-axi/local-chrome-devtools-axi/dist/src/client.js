 ...
 */
export async function checkBridgeHealth(port, opts = {}) {
    try {
 ...
 * expires).
 */
export async function terminateBridgeProcess(pid, opts = {}) {
    if (!isProcessAlive(pid))
        return;
...
export async function ensureBridge() {
    ...
    if (sawShallowReady) {
        throw new CdpError("Bridge is running but the attached CDP target appears to have gone away", "BRIDGE_NOT_READY", [
    ...
    const suggestions = [
        "Check that chrome-devtools-mcp is installed: npx chrome-devtools-mcp@1.4.0 --help",
    ];
    if (usingNpx) {
        suggestions.push("If `npx -y chrome-devtools-mcp@1.4.0` is slow on this machine, install mcp globally and set:", '  export CHROME_DEVTOOLS_AXI_MCP_PATH="$(npm prefix -g)/lib/node_modules/chrome-devtools-mcp/build/src/bin/chrome-devtools-mcp.js"');
    }
    suggestions.push("Or extend the deadline: export CHROME_DEVTOOLS_AXI_BRIDGE_TIMEOUT_MS=60000");
    throw new CdpError(`Bridge failed to start within ${seconds}s`, "BRIDGE_NOT_READY", suggestions);
}
/**
