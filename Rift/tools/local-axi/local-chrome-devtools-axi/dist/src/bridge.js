 ...
 */
export async function isBridgeTargetReachable(client) {
    try {
...
export function buildTransportArgs() {
    // LOCAL-AXI PATCH: pinned from "chrome-devtools-mcp@latest" to a fixed
    // version so a runtime `npx` never silently fetches an unaudited browser
    // automation server (which pulls Chromium). Bump deliberately + re-audit.
    // To avoid any download, set CHROME_DEVTOOLS_AXI_BROWSER_URL to attach to
    // system Chrome (see ../AUDIT.md).
    const args = ["-y", "chrome-devtools-mcp@1.4.0"];
        ...
        if (isWs) {
            ...
            if (wsHeaders) {
                ...
                if (parsedHeaders === null ||
                    ...
                    throw new Error("CHROME_DEVTOOLS_AXI_WS_HEADERS must be a JSON object");
                }
 ...
 */
export function detectGlobalMcpPath(probe = DEFAULT_MCP_PATH_PROBE) {
    const prefix = probe.getNpmPrefix();
    if (!prefix || prefix.length === 0)
        return null;
    const candidate = join(prefix, "lib", "node_modules", "chrome-devtools-mcp", "build", "src", "bin", "chrome-devtools-mcp.js");
 ...
 */
export function resolveTransportSpec(probe = DEFAULT_MCP_PATH_PROBE) {
    const mcpArgs = buildTransportArgs();
    const explicit = process.env.CHROME_DEVTOOLS_AXI_MCP_PATH;
    const mcpPath = explicit && explicit.length > 0 ? explicit : detectGlobalMcpPath(probe);
    if (mcpPath) {
        // Strip the npx prefix `["-y", "chrome-devtools-mcp@1.4.0"]` — direct
