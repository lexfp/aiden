...
window.G = {
    ...
    startStoryline() {
        // Homescreen-only Storyline entrypoint. Loads/starts the storyline module.
        this.selMode = 'storyline';
        if (window.Storyline && typeof window.Storyline.start === 'function') {
            window.Storyline.start();
            return;
        }
        // Dynamically load the storyline script if not present
        const s = document.createElement('script');
        s.src = 'js/storyline.js';
        s.onload = () => { if (window.Storyline && typeof window.Storyline.start === 'function') window.Storyline.start(); };
        s.onerror = () => { this.showNotif('Failed to load Storyline module'); };
